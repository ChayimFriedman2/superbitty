...
/// and big enough to hold the enums.
///
/// ```
...
/// An enum that can be used as a bitfield. This is usually [derived].
///
/// # Safety
///
///   - An enum implementing this trait must not carry payload.
///   - The [`SHIFT`] and [`BITS_LEN`] values must be correct.
///   - Variants must not have negative discriminants.
///   - Enums must be inhabited, that is, have at least one variant.
///   - [`into_raw()`] must return a valid discriminant (something you
///     can feed back to [`from_raw()`]).
///
...
pub unsafe trait BitFieldCompatible: Copy {
    /// The number we need to left-shift with to reach a valid variant from a compressed
    /// value which has all trailing zeros trimmed.
    ///
    /// For example, for the discriminants `0b01000`, `0b10000` and `0b11000`, the
    /// value of [`BITS_LEN`] will be 2 and the value of `SHIFT` will be 3, because
    /// we need to left-shift three times to go from `0b01`, `0b10` and `0b11` to
    /// the corresponding variants.
    ///
    /// [`BITS_LEN`]: BitFieldCompatible::BITS_LEN
    const SHIFT: u32;

    /// The minimum number of bits this enum can occupy.
    ///
    /// If there is only one variant, and it is not zero, this must be 1 even though
    /// technically zero bits suffice. If the variant is zero this can be zero.
    const BITS_LEN: u32;
    ...
    /// Retrieves the discriminant of the value.
    fn into_raw(self) -> u128;

    /// Converts a discriminant into the enum.
    ///
    /// # Safety
    ///
    /// `v` must be a valid discriminant
